...
pub fn validate_legacy_message_cheap(
    msg: Value,
    id: Multihash,
    sequence: Option<LegacyF64>,
    timestamp: LegacyF64
) -> Result<()> {
...
mod tests {
    ...
    #[test]
    fn it_validates_a_message_with_many_euros() {
        let result = validate_message_hash_chain::<_, &[u8]>(
            MESSAGE_WITH_EUROS.as_bytes(),
            None,
        );
  ...
  "timestamp": 1571140555382.002
    ...
    const MESSAGE_WITH_EUROS: &str = r##"{
  "key": "%xS36toz/QgfHh0EtfGo3sa8kdTgxO2G5JQGj6L9VNBs=.sha256",
  "value": {
    "previous": null,
    "sequence": 1,
